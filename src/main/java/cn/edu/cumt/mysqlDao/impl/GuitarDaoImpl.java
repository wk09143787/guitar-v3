...
import cn.edu.cumt.dao.DataAccess;
...
public class GuitarDaoImpl implements GuitarDao {
	...
	public boolean add(Guitar guitar) {
		...
		return MysqlDbUtil.executeUpdate("INSERT INTO GUITAR VALUES (?, ?, ?);",
	...
	public Guitar getBySerialNumber(String serialNumber) {
		Guitar guitar = null;
		ResultSet rs = MysqlDbUtil.executeQuery("SELECT * FROM GUITAR WHERE serialNumber=?",
	...
	public boolean delete(String serialNumber) {
		return MysqlDbUtil.executeUpdate("DELETE FROM GUITAR WHERE serialNumber=?",
				new Object[]{serialNumber});
	}
